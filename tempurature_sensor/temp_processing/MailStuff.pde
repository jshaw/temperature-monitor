...
void sendMail(int temp) {
  ...
  try
  {
    ...
    message.setFrom(new InternetAddress("jrdnshw@gmail.com", "Jordan Shaw"));

    // Who is this message to (we could do fancier things like make a list or add CC's)
    message.setRecipients(Message.RecipientType.TO, InternetAddress.parse("jrdnshw@gmail.com, matt.stubbings@gmail.com, codey@torontomactech.com", false));

    // Subject and body
    message.setSubject("HEAT WARNING!");
    message.setText("This is an automated warning that the upstairs 3rd floor at 91 Dovercourt Rd. Toronto is becomming unlivable with a temp of " + temp  +"! Please turn on the AC AS SOON AS POSSIBLE! Thanks!");
